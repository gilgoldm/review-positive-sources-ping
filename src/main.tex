...
I believe that this is an important and appropriate topic for
\textit{Mathematics of Computation}. Super-resolution, line spectral estimation,
...
The manuscript is therefore of interest to a substantial audience, not merely a narrow niche.
It also fits the scope of the journal well: the paper addresses a computationally meaningful notion of resolution limit,
...
This seems to be the main genuinely new result relative to the authors' earlier works on general sources.
The final lower bound is not only of the correct asymptotic order,
but is presented with explicit and fairly sharp constants.
...
which is mathematically meaningful and practically relevant.

The paper contains several additional useful contributions.
The support-recovery lower bound and instability constructions
help show that the upper bounds are essentially optimal in order.
The multi-dimensional extension is natural and valuable.
I also found the observation in Section 4 interesting:
positivity does not necessarily help resolution, and for two sources with different
phases the resolution limit can in fact be better than in the positive case.
Finally, the link to sparsity-promoting algorithms and the numerical verification of
phase transitions strengthen the computational relevance of the manuscript.
...
I believe the paper is publishable. My recommendation is:

\begin{center}
\textbf{Accept subject to minor revisions.}
\end{center}
